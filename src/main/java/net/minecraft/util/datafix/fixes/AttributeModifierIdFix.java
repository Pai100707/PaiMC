...
public class AttributeModifierIdFix extends DataFix {
   ...
   private static Stream<Dynamic<?>> fixModifiersTypeWrapper(Stream<?> $$0) {
      return fixModifiers((Stream<Dynamic<?>>)$$0);
   }
      ...
      return $$0.update(
         ...
         $$0x -> $$0x.update(
            ...
            $$0xx -> (Dynamic)DataFixUtils.orElse(
               $$0xx.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map($$0xx::createList), $$0xx
            )
         )
      );
   }
