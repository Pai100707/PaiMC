...
public class DebugSubscriptions<T> {
   ...
   public static final DebugSubscription<net.minecraft.util.Unit> VILLAGE_SECTIONS = registerWithValue("village_sections", net.minecraft.util.Unit.STREAM_CODEC);
   ...
   private static DebugSubscription<?> registerSimple(String $$0) {
      return (DebugSubscription<?>)Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace($$0), new DebugSubscription(null));
   }

   private static <T> DebugSubscription<T> registerWithValue(String $$0, StreamCodec<? super RegistryFriendlyByteBuf, T> $$1) {
      return (DebugSubscription<T>)Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace($$0), new DebugSubscription($$1));
   }

   private static <T> DebugSubscription<T> registerTemporaryValue(String $$0, StreamCodec<? super RegistryFriendlyByteBuf, T> $$1, int $$2) {
      return (DebugSubscription<T>)Registry.register(
         BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace($$0), new DebugSubscription($$1, $$2)
      );
   }
}
