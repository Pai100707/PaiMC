...
public record JfrStatsResult(
   ...
   public List<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>> chunkGenSummary() {
      Map<ChunkStatus, List<ChunkGenStat>> $$0 = this.chunkGenStats.stream().collect(Collectors.groupingBy(ChunkGenStat::status));
      return $$0.entrySet()
         .stream()
         .map($$0x -> Pair.of((ChunkStatus)$$0x.getKey(), TimedStatSummary.summary((List)$$0x.getValue())))
         .filter($$0x -> ((Optional)$$0x.getSecond()).isPresent())
         .map($$0x -> Pair.of((ChunkStatus)$$0x.getFirst(), (TimedStatSummary)((Optional)$$0x.getSecond()).get()))
         .sorted(
            Comparator.<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>, Duration>comparing($$0x -> ((TimedStatSummary)$$0x.getSecond()).totalDuration())
               .reversed()
         )
         .toList();
   }
