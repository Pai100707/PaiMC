...
public class VanillaRegistries {
   ...
   private static void validateThatAllBiomeFeaturesHaveBiomeFilter(Provider $$0) {
   ...
   public static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderGetter<PlacedFeature> $$0, HolderLookup<Biome> $$1) {
      $$1.listElements().forEach($$1x -> {
         Identifier $$2 = $$1x.key().identifier();
         List<HolderSet<PlacedFeature>> $$3 = ((Biome)$$1x.value()).getGenerationSettings().features();
         $$3.stream().<Holder>flatMap(HolderSet::stream).forEach($$3x -> $$3x.unwrap().ifLeft($$2xx -> {
            Reference<PlacedFeature> $$3xx = $$0.getOrThrow($$2xx);
            if (!validatePlacedFeature((PlacedFeature)$$3xx.value())) {
               Util.logAndPauseIfInIde("Placed feature " + $$2xx.identifier() + " in biome " + $$2 + " is missing BiomeFilter.biome()");
            }
         }).ifRight($$1xxx -> {
   ...
   public static Provider createLookup() {
      Frozen $$0 = RegistryAccess.fromRegistryOfRegistries(BuiltInRegistries.REGISTRY);
      Provider $$1 = BUILDER.build($$0);
