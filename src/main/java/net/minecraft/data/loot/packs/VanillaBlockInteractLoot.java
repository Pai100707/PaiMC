...
public record VanillaBlockInteractLoot(Provider registries) implements LootTableSubProvider {
   @Override
                     ...
                     LootItem.lootTableItem(Items.SWEET_BERRIES)
                        ...
                        .when(
                           LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.SWEET_BERRY_BUSH)
                              .setProperties(
                                 net.minecraft.advancements.criterion.StatePropertiesPredicate.Builder.properties().hasProperty(SweetBerryBushBlock.AGE, 3)
                              )
                        )
                  )
            )
