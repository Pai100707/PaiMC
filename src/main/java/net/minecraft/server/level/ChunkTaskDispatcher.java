...
public class ChunkTaskDispatcher implements ChunkHolder.LevelChangeListener, AutoCloseable {
   ...
   public void onLevelChange(ChunkPos $$0, IntSupplier $$1, int $$2, IntConsumer $$3) {
      this.dispatcher.schedule(new RunnableWithPriority(0, () -> {
         int $$4 = $$1.getAsInt();
         if (SharedConstants.DEBUG_VERBOSE_SERVER_EVENTS) {
            LOGGER.debug("RES {} {} -> {}", new Object[]{$$0, $$4, $$2});
         }
   ...
   public void release(long $$0, Runnable $$1, boolean $$2) {
      this.dispatcher.schedule(new RunnableWithPriority(1, () -> {
   ...
   public void submit(Runnable $$0, long $$1, IntSupplier $$2) {
      this.dispatcher.schedule(new RunnableWithPriority(2, () -> {
         int $$3 = $$2.getAsInt();
         if (SharedConstants.DEBUG_VERBOSE_SERVER_EVENTS) {
            LOGGER.debug("SUB {} {} {} {}", new Object[]{new ChunkPos($$1), $$3, this.executor, this.queue});
         }
   ...
   protected void pollTask() {
      this.dispatcher.schedule(new RunnableWithPriority(3, () -> {
   ...
   protected void scheduleForExecution(ChunkTaskPriorityQueue.TasksForChunk $$0) {
      CompletableFuture.allOf($$0.tasks().stream().map($$0x -> this.executor.scheduleWithResult($$1 -> {
         $$0x.run();
         $$1.complete(Unit.INSTANCE);
      })).toArray(CompletableFuture[]::new)).thenAccept($$0x -> this.pollTask());
   }
