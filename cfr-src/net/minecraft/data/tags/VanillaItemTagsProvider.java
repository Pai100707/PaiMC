...
extends IntrinsicHolderTagsProvider<Item> {
    ...
    protected void addTags(HolderLookup.Provider $$02) {
        ...
        this.tag(ItemTags.TRIM_MATERIALS).addAll($$02.lookupOrThrow(Registries.ITEM).listElements().filter($$0 -> ((Item)$$0.value()).components().has(DataComponents.PROVIDES_TRIM_MATERIAL)).sorted(Comparator.comparing($$0 -> $$0.key().identifier())).map(Holder.Reference::value));
